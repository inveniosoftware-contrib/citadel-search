...
def test_file_processed_listener(
    ...
    file_processed_listener(app=base_app, processor_id="some-processor", file=file.obj, data=data)

    record_from_object_version_mock.assert_called_once_with(file.obj)
    persist_file_content_mock.assert_called_once_with(record, data, file.obj.basename)
...
def test_record_deleted_listener(delete_all_record_files_mock, base_app, record_with_file):
    """Test process file calls."""
    record = CernSearchRecord.get_record(record_with_file.id)
